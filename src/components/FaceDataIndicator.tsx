...
export default function FaceDataIndicator() {
  ...
  if (storageStatus.face.hasFaceData) {
    indicators.push(
      <span 
        key="face" 
        className="text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded border cursor-pointer hover:bg-gray-100"
  ...
  if (storageStatus.avatar.hasCustomSelection && storageStatus.avatar.avatarId) {
    ...
    indicators.push(
      <span key="avatar" className="text-xs text-gray-500 bg-blue-50 px-2 py-1 rounded border border-blue-200">
        🎭 選択アバター: {avatarInfo?.name || 'Unknown'}
      </span>
    );
  } else if (storageStatus.avatar.isDefaultAvatar) {
    indicators.push(
      <span key="default" className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded border">
        🎭 デフォルトアバター
      </span>
    );
  }
