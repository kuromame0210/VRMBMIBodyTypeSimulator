    ...
    <div className="w-full space-y-4">
        ...
        <div className="p-4">
          <div className="mb-4">
            <h4 className="text-sm font-medium text-black mb-2">
              {searchTerm ? (
                <>🔍 検索結果: "{searchTerm}" <span className="text-xs text-black">({getAllFilteredShapes().length} 個)</span></>
              ) : (
                <>
              ...
              return (
                <div key={shapeName} className={`space-y-1 p-3 border rounded-lg hover:bg-gray-50 ${isLikelyLimitedEffect ? 'border-yellow-300 bg-yellow-50' : ''}`}>
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-black pr-2 flex items-center">
                      {shapeName}
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-xs text-black min-w-[3rem] text-right">
      ...
      {false && (
        <div className="border rounded-lg p-4">
          <h3 className="text-sm font-medium text-black mb-4">プリセット管理</h3>
          ...
          <div className="space-y-2">
            {store.savedPresets.length === 0 ? (
              <p className="text-black text-sm">保存されたプリセットはありません</p>
            ) : (
              store.savedPresets.map(preset => (
                ...
                >
                  <div>
                    <div className="font-medium text-black text-sm">{preset.name}</div>
