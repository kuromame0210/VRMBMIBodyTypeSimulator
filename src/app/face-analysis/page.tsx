...
function FaceAnalysisContent() {
  ...
  const [, setIsWarmedUp] = useState(false);
  ...
  useEffect(() => {
    // 初期ロード中は保存しない
    if (!photoFeatures || !uploadedImage) {
      // 自動保存スキップ - データ不足
      return;
    }
    
    // BlendShape値のバリデーション
    // BlendShape値の存在チェック（将来の機能拡張用）
    // const hasBlendShapeData = blendShapeStore.currentValues && Object.keys(blendShapeStore.currentValues).length > 0;
    // BlendShape値のバリデーション完了
    
    // BlendShape値が変更された時に自動保存
    const saveCurrentState = async () => {
      try {
        // 自動保存実行
        const faceData = createFaceFeatureData(
          photoFeatures,
          blendShapeStore.currentValues,
          uploadedImage
        );
        ...
        // UI更新（控えめに）
        setStatus(`💾 調整内容を自動保存しました (${new Date().toLocaleTimeString()})`);
      } catch {
      }
    };

    // 500ms のデバウンス付きで保存
    const timeoutId = setTimeout(saveCurrentState, 500);
    return () => clearTimeout(timeoutId);
  }, [blendShapeStore.currentValues, photoFeatures, uploadedImage]); // BlendShape値の変更を監視
  ...
  const calculateDetailedFeatures = (landmarks: Array<{x: number, y: number, z?: number}>, processingTime: number): FaceFeatures => {
    ...
    try {
      ...
      // const eyeAspectRatio = eyeHeight / eyeWidth; // 将来の使用予定
      ...
      // const leftBrowMiddle = landmarks[107];
      // const leftBrowOuter = landmarks[55];
            ...
            <div className="mt-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 max-h-80 overflow-y-auto">
                <div className="space-y-2">
                  ...
                  {/* 解析情報 */}
                  <div>
                    <h4 className="font-medium text-black mb-2">ℹ️ 解析情報</h4>
